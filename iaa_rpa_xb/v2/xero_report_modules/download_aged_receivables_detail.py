...
Aged Receivables Detail report from Xero Blue: resolving the report end date,
...
Inputs are modelled as a dataclass:
    AgedReceivablesRequest - everything one download needs (report config +
                             file/output config). The live browser/engine is
                             passed separately to the download function.

Structure:
    The orchestrator (download_aged_receivables_detail_report) owns the order of
...
Timeouts:
    DEFAULT_ELEMENT_TIMEOUT - general element waits. Overridable per run via
        AgedReceivablesRequest.element_timeout.
...
How to call:
    ...
    request = AgedReceivablesRequest(
        ...
        report_file_name="ACME_Receivables_2024",
        # end_date="31 Dec 2024",               # optional; defaults to 30 Jun {FY}
        # add_gst_column=True,                   # optional, default False
        # window_title="Aged Receivables Detail",# optional, has a default
...
Note on the financial year:
    `financial_year` follows the Australian convention: FY "2024" runs from
    1 Jul 2023 to 30 Jun 2024. When `end_date` is omitted, the report end date
    defaults to "30 Jun {financial_year}".

Failure behaviour:
    Errors are logged (by ``ProcessLogger``) and RE-RAISED, so the caller can
    detect failure. To swallow instead, wrap the ``with ProcessLogger(...)``
    block in a try/except that returns rather than propagates.
"""
...
AgingMethod = Literal["Due Date", "Invoice Date", "Transaction Date"]
...
class AgedReceivablesRequest:
    ...
    Attributes:
        financial_year:     FY end year as an int (e.g. 2024). Used to
                            derive the default date range when ``end_date`` is
                            omitted.
    ...
    def default_start_date(self) -> str:
        """Start of the default FY range, "1 Jul {financial_year - 1}". For
        logging only - Xero only needs the end date."""
    ...
    def dest_path(self) -> str:
        """Full save path, e.g. ".../ACME_Receivables_2024.xlsx". Tolerates an
...
def download_aged_receivables_detail_report(
    ...
    Args:
        ...
        request (AgedReceivablesRequest): All configuration for the download.
    ...
    Raises:
        ...
        timing). Wrap the context block in a try/except to swallow instead.
    """
    with ProcessLogger("Xero Blue Download Aged Receivables Detail Report", logger):
        ...
        for line in request.summary_lines():
            logger.info(line)
        _log_resolved_date_range(request)
...
def _log_resolved_date_range(request: AgedReceivablesRequest) -> None:
...
def configure_report_dates_and_aging(browser, request: AgedReceivablesRequest) -> None:
    """
    ...
    Args:
        ...
        request (AgedReceivablesRequest): Supplies the resolved end date,
    ...
    custom_date_locator = "xpath://input[@id='report-settings-custom-date-input-to']"
...
def configure_gst_column(browser, request: AgedReceivablesRequest) -> None:
    ...
    Args:
        browser: SeleniumBrowser wrapper instance.
        request (AgedReceivablesRequest): Supplies the GST flag and element_timeout.
...
def update_and_export_report(browser, request: AgedReceivablesRequest) -> None:
    ...
    Args:
        browser: SeleniumBrowser wrapper instance.
        request (AgedReceivablesRequest): Supplies window title and dest path.
