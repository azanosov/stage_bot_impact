"""
Module for downloading Aged Receivables Summary reports from Xero Blue.

The Summary sibling of Aged Receivables Detail: an "as at" report that ages
receivables by Due Date or Invoice Date, optionally adds the Outstanding GST
...
which raises ``DataExtractionError`` if a (valid) option is absent on the page - kept
...
How to call:
    ...
    from download_aged_receivables_summary import (
        AgedReceivablesSummaryRequest, download_aged_receivables_summary_report,
    )

    request = AgedReceivablesSummaryRequest(
        financial_year=2024,
        aging_by="Due Date",
        download_directory=r"C:\\Reports",
        report_file_name="aged_receivables_summary_2024",
    ...
    download_aged_receivables_summary_report(browser, request)
...
_AGEING_PERIOD_RE = re.compile(
    r"^\s*(\d+)\s+periods?\s+of\s+(\d+)\s+(\w+)\s*$", re.IGNORECASE
)
...
class AgedReceivablesSummaryRequest:
    """Everything needed to download one Aged Receivables Summary report.
    ...
    window_title: str = "Aged Receivables Summary"
...
def download_aged_receivables_summary_report(
    browser, request: AgedReceivablesSummaryRequest
) -> str:
    """
    Download an Aged Receivables Summary report from Xero Blue.
...
def configure_report_date(browser, request: AgedReceivablesSummaryRequest) -> None:
...
def select_ageing_by(browser, request: AgedReceivablesSummaryRequest) -> None:
...
def set_ageing_period(browser, request: AgedReceivablesSummaryRequest) -> None:
...
def add_gst_column_to_report(browser, request: AgedReceivablesSummaryRequest) -> None:
...
def update_and_export_report(browser, request: AgedReceivablesSummaryRequest) -> str:
    ...
    if not browser.does_page_contain_element(
        ...
        logger.warning("Export button not found - no receivables data for this client")
        raise DataExtractionError(
            "No Aged Receivables Summary data available for this client."
        )
