"""
Module for downloading Payroll Activity Summary reports from Xero Blue.
...
How to call:
    ...
    request = PayrollActivitySummaryRequest(
        download_directory=r"C:\\Reports",
        report_file_name="payroll_activity_summary_2024",
    ...
    download_payroll_activity_summary_report(browser, request)
...
class PayrollActivitySummaryRequest:
    """Everything needed to download one Payroll Activity Summary report.
    ...
    window_title: str = "Payroll Activity Summary"
...
def download_payroll_activity_summary_report(browser, request: PayrollActivitySummaryRequest) -> None:
    """
    Download a Payroll Activity Summary report from Xero Blue.
    ...
    """
    with ProcessLogger("Xero Blue Download Payroll Activity Summary Report", logger):
...
def configure_report_dates(browser, request: PayrollActivitySummaryRequest) -> None:
...
def update_and_export_report(browser, request: PayrollActivitySummaryRequest) -> None:
    ...
    common.capture_report_screenshot(
        browser, request.screenshot_path, "payroll_activity_summary", "before_update",
    ...
    common.capture_report_screenshot(
        browser, request.screenshot_path, "payroll_activity_summary", "after_update",
        enabled=request.capture_screenshots,
    )
