...
Configures and downloads an Aged Payables Detail report: enters the report end
...
Inputs are modelled as a dataclass:
    AgedPayablesRequest - everything one download needs. The live browser/engine
...
How to call:
    ...
    request = AgedPayablesRequest(
        ...
        report_file_name="aged_payables_2024",
        aging_by="Due Date",
...
def configure_report_date(browser, request: AgedPayablesRequest) -> None:
...
def select_ageing_by(browser, request: AgedPayablesRequest) -> None:
...
def update_and_export_report(browser, request: AgedPayablesRequest) -> None:
    ...
    common.capture_report_screenshot(
        browser, request.screenshot_path, "aged_payables_detail", "before_update",
    ...
    if not browser.does_page_contain_element(config.SH_EXPORT_BUTTON, timeout=common.DEFAULT_ELEMENT_TIMEOUT):
        logger.warning("Export button not found - no Aged Payables data available for this client")
        raise DataExtractionError("No Aged Payables Detail data available for this client.")
    ...
    common.capture_report_screenshot(
        browser, request.screenshot_path, "aged_payables_detail", "after_update",
        enabled=request.capture_screenshots,
    )
