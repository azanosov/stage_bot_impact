...
def navigate_to_dashboard_page(browser) -> None:
    ...
    try:
        browser.click_element(
            config.NAV_HOME_LINK, timeout=common.DEFAULT_ELEMENT_TIMEOUT
        )
        if browser.does_page_contain_element(
            config.NAV_HOME_MAIN, timeout=common.DEFAULT_ELEMENT_TIMEOUT
        ):
    ...
    try:
        browser.click_element(
            config.NAV_DASHBOARD_LINK, timeout=common.DEFAULT_ELEMENT_TIMEOUT
        )
...
def navigate_to_all_reports_page(browser) -> None:
    ...
    try:
        browser.click_element(
            config.REPORTS_REPORTING_MENU_BUTTON, timeout=common.DEFAULT_ELEMENT_TIMEOUT
        )
        logger.info("Opened the Reporting menu")
        browser.click_element(
            config.REPORTS_ALL_REPORTS_LINK, timeout=common.DEFAULT_ELEMENT_TIMEOUT
        )
        if browser.does_page_contain_element(
            config.REPORTS_CENTRE_PARENT, timeout=common.DEFAULT_ELEMENT_TIMEOUT
        ):
        ...
        logger.warning(
            "New-UI All reports link not found - trying legacy Accounting path"
        )

    # Fallback: legacy.
    try:
        browser.click_element(
            config.REPORTS_ACCOUNTING_TAB, timeout=common.DEFAULT_ELEMENT_TIMEOUT
        )
        browser.click_element(
            config.REPORTS_ACCOUNTING_REPORTS_LINK,
            timeout=common.DEFAULT_ELEMENT_TIMEOUT,
        )
        # No legacy landing-page marker available yet, so we can't confirm success.
        logger.info(
            "Clicked the legacy Accounting -> Reports link (navigation not verified)"
        )
    except ElementNotFoundError as e:
...
def navigate_to_invoices_page(browser) -> None:
    ...
    try:
        if browser.does_page_contain_element(
            config.INV_NAV_SALES_BUTTON, timeout=timeout
        ):
