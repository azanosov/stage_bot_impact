...
from iaa_rpa_xero_blue.navigation import (
...
from iaa_rpa_xero_blue.download_profit_and_loss import (
...
def impact_main() -> None:
    ...
    reports_to_process = [
        {
            ...
            "fn": download_activity_statement_report,
            "request": ActivityStatementRequest(
                period=StatementPeriod(
                    "September", 2025
                ),  # StatementPeriod, not a tuple
    ...
    for report in reports_to_process:
        ...
        if report["name"] == "Bank Reconciliation" and args is not None:
            ...
            if not accounts:
                logger.warning(
                    f"{report['name']}: no bank accounts to process — skipping"
                )
                continue
            base = report["request"]
