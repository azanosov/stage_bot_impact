...
def send_email(
    EmailConfig: dict,
    sender: str,
    to_addresses: str | tuple | list,
    cc_addresses: str | tuple | list,
    subject: str,
    body: str,
    attachment_path: str | list[str] | None = None,
):
    """
    ...
    try:
        ...
        if not all([tenant_id, client_id, client_secret]):
            raise ConfigurationError(
                "Missing required email configuration: TenantId, ClientId, or ClientSecret"
            )
    ...
    try:
        ...
        raise EmailNotificationError(
            f"Failed to authenticate with Microsoft Graph API: {e}"
        ) from e
    ...
    if "access_token" in token_response:
        ...
        try:
            response = requests.post(
                email_endpoint, headers=headers, json=email_message, timeout=30
            )
...
def prepare_attachment(file_path: str) -> dict:
    ...
    return {"filename": Path(file_path).name, "content_base64": content}
